...
@Component({
  selector: 'app-nft-card-bkcn',
  templateUrl: './nft-card-bkcn.component.html',
  styleUrls: ['./nft-card-bkcn.component.scss']
})
export class NftCardBkcnComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.nftName.length > 30) {
      ...
      this.nftName = this.nftName + '...';
    }

    if (this.nft.marketplace.type === 'listing-auction') {
      const max = Math.max.apply(
        Math,
        this.nft.auction.bids.map((bid) => bid.bidAmount)
      );
      this.maxBid = this.nft.auction.bids.filter((bid) => bid.bidAmount === max)[0];
  ...
  openNft() {
    ...
    this.router.navigate([`nft/${contractAddress}/${tokenId}`]);
  }
  ...
  openDetail() {
    this.router.navigateByUrl(`/profile/nft-detail/${this.nft.contract.address}/${this.nft.tokenId}`);
  }
